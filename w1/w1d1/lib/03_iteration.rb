...
def substrings(string)
  substring_array = []
  0.upto(string.length) do |start_idx|
    start_idx.upto(string.length) do |end_idx|
...
def subwords(word, dictionary)
  ...
  substring_array.select {|el| dictionary.include?(el) }.uniq
...
class Array
  ...
  def my_inject(&blk)
    total = self[0]
    self[1..-1].my_each { |el| total = blk.call(total,el)}
    total
  end
end
